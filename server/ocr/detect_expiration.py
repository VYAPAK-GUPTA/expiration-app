...
def detect_expiration_date(image_path):
    ...
    image = enhancer.enhance(2)  # Increase contrast for better text recognition
            ...
            for pattern in date_patterns:
                ...
                if matches:
                    ...
                    for date_str in matches:
                        ...
                        for fmt in ('%d/%m/%y', '%d/%m/%Y', '%d.%m.%y', '%d.%m.%Y', '%d-%m-%y', '%d-%m-%Y', '%Y-%m-%d', '%d %B %Y'):
                            try:
                                # Parse the date and return it if it's after today's date
                                expiration_date = datetime.strptime(date_str, fmt).date()
                                print(f"Detected expiration date: {expiration_date}")  # Debugging: valid expiration date
